...
def salvar_no_google_sheets(dados_auditoria):
    try:
        ...
        # Puxa o dicionário gerado limpo pelo Secrets
        creds_dict = dict(st.secrets["gcp_service_account"])
        # Trava de segurança para garantir a quebra de linha
    ...
    if senha == "1234":
        ...
        if not df_historico.empty and len(df_historico.columns) >= 19:
            ...
            if loja_busca != "Selecione...":
                ...
                st.info(f"Ao reprocessar a auditoria de {data_selecionada}, o sistema fará a leitura matemática novamente, recriando os PDFs e disparando um novo e-mail com as regras de pontuação atuais.")
                ...
                if st.button("Reprocessar e Reenviar Auditoria", type="primary"):
                    ...
                    try:
                        ...
                        st.error(f"Erro ao reprocessar: {e}. Isso geralmente ocorre se você selecionar uma leitura antiga, pois ela não possui os dados brutos das frentes.")
        else:
